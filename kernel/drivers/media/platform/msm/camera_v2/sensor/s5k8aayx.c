...
static struct msm_sensor_power_setting s5k8aayx_power_setting_dum[] = {
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VANA,
		.config_val = 0,
...
//Preview config[0] 1280x960  30fps~7.5fps
	...
	{0x002A, 0x01E8},
#if defined(CONFIG_MACH_LYNX_DL45) || defined(CONFIG_MACH_TAC)
...
{
#if 1
	return msm_sensor_i2c_probe(client, id, &s5k8aayx_s_ctrl);
#else
	int rc = 0;
...
static int32_t s5k8aayx_platform_probe(struct platform_device *pdev)
{
	...
	rc = msm_sensor_platform_shortcut_probe(pdev, match->data);
	return rc;
}
...
static int __init s5k8aayx_init_module(void)
{
	int32_t rc = 0;
	pr_info("%s:%d\n", __func__, __LINE__);
	perf_lock_init(&s5k8aayx_perf_lock, PERF_LOCK_787200KHz, "camera_s5k8aayx");
	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	int cur_size, array_size;
       ...
       CDBG("%s:%d hospire-kernel size of kzalloc %d\n", __func__, __LINE__, size *(sizeof(struct msm_camera_i2c_seq_reg_array)));
...
int32_t s5k8aayx_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(hw_rev >= SH_REV_ES1) {
		...
		s5k8aayx_s_ctrl.power_setting_array.power_setting = s5k8aayx_power_setting_dum;
		s5k8aayx_s_ctrl.power_setting_array.size = ARRAY_SIZE(s5k8aayx_power_setting_dum);
	}
#ifdef CONFIG_MACH_TAC
	s5k8aayx_s_ctrl.power_setting_array.power_setting = s5k8aayx_power_setting;
	s5k8aayx_s_ctrl.power_setting_array.size = ARRAY_SIZE(s5k8aayx_power_setting);
#endif
