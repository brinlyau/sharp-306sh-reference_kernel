...
static struct perf_lock s5k4h5yb_perf_lock;
...
static struct msm_sensor_power_setting s5k4h5yb_power_setting_dum[] = {
	{
		.seq_type = SENSOR_VREG,
		.seq_val = CAM_VDIG,
		.config_val = 0,
...
int32_t s5k4h5yb_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(hw_rev >= SH_REV_PP1) {
		...
		s5k4h5yb_s_ctrl.power_setting_array.size = ARRAY_SIZE(s5k4h5yb_power_setting);
	} else {
		s5k4h5yb_s_ctrl.power_setting_array.power_setting = s5k4h5yb_power_setting_dum;
		s5k4h5yb_s_ctrl.power_setting_array.size = ARRAY_SIZE(s5k4h5yb_power_setting_dum);
	}
...
int32_t s5k4h5yb_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	perf_unlock(&s5k4h5yb_perf_lock);
...
static int32_t s5k4h5yb_platform_probe(struct platform_device *pdev)
{
	...
	if(rc < 0){
		...
		/* OTP read */
		struct msm_sensor_ctrl_t *s_ctrl = (struct msm_sensor_ctrl_t *)match->data;
		int i = 0;
