...
#define FLASH_INT_EN(base)				(base + 0x15)
...
#define	FLASH_INT_EN_MASK				0xFF
#define	FLASH_ALT_RAMP_DN_MASK		0xFF
#define	FLASH_VPH_PWR_DROOP_MASK	0xF3
#define	FLASH_INT_EN_VAL				0xFF
#define	FLASH_WATCHDOG_TIMER_VAL	0x00
#define	FLASH_ALT_RAMP_DN_VAL		0x00
#define	FLASH_VPH_PWR_DROOP_VAL		0xF0
#define FLASH_SAFETY_TIMER_VAL		0x63
#define FLASH_CLAMP_CURR_VAL		0x03
#define FLASH_CLAMP_CURR_VAL2		0x0f
#define FLASH_STARTUP_DELAY_VAL		0x03
#define FLASH_ENABLE_CONTROL_VAL0	0x60
#define FLASH_ENABLE_CONTROL_VAL1	0x80
#define FLASH_ENABLE_CONTROL_VAL2	0xc0
#define FLASH_LED_STROBE_CTRL_VAL0	0x00
#define FLASH_LED_STROBE_CTRL_VAL	0x80
#define FLASH_DISABLE_ALL1				0x60
...
regulator_turn_off:
	if (led->flash_cfg->torch_on) {
		rc = qpnp_led_masked_write(led,	FLASH_ENABLE_CONTROL(led->base),
				FLASH_ENABLE_MODULE_MASK, FLASH_DISABLE_ALL1);
		if (rc) {
			dev_err(&led->spmi_dev->dev,
				"Enable reg write failed(%d)\n", rc);
		}
...
static int qpnp_flash_set(struct qpnp_led_data *led)
{
	...
	if (val > 0) {
		if (led->flash_cfg->torch_enable) {
			#if defined(CONFIG_ARCH_MSM8226)
			tmp8 = (u8)led->flash_cfg->current_prgm;
			if(tmp8 == 0)
				tmp8 = 1;
			else if(tmp8 == 9)
				tmp8 = 19;
			...
			if(tmp8 <= 1) {
				...
				if (rc) {
					dev_err(&led->spmi_dev->dev,
					"qpnp_torch_control_enable failed(%d)\n",
					rc);
					return rc;
				}
			}
			else {
				rc = qpnp_flash_control_enable(true);
				if (rc) {
					dev_err(&led->spmi_dev->dev,
					"qpnp_flash_control_enable failed(%d)\n",
					rc);
					return rc;
				}
			}
...
//			rc = qpnp_flash_control_enable(false);
//			if (rc) {
//				dev_err(&led->spmi_dev->dev,
//				"qpnp_flash_control_enable failed(%d)\n",
//				rc);
...
/* SHLOCAL_CAMERA_DRIVERS-> */
	rc = qpnp_led_masked_write(led, FLASH_INT_EN(led->base),
		FLASH_INT_EN_MASK, FLASH_INT_EN_VAL);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"Flash Intr reg write failed(%d)\n", rc);
		return rc;
	}
