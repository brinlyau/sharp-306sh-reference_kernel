...
#ifdef CONFIG_USB_MSM_OTG_READ_USB_ID_FROM_SHBATT
 	#include <sharp/shbatt_kerl.h>
#endif /* CONFIG_USB_MSM_OTG_READ_USB_ID_FROM_SHBATT */
...
static int msm_otg_notify_chg_info(struct usb_phy *phy, unsigned mA)
{
	...
	dev_info(motg->phy.dev, "msm_otg_notify_chg_info chg_type=%d chg_devtype=%d mA=%d vbus=%d \n"
				, (int)motg->chg_type, type, mA, test_bit(B_SESS_VLD, &motg->inputs));
	return 0;
...
void android_notify_chager_info(unsigned mA)
{
	struct msm_otg *motg = the_msm_otg;
	if (!motg || !(motg->phy.otg))
		return;
	msm_otg_notify_chg_info(motg->phy.otg->phy, mA);
	return;
}
EXPORT_SYMBOL(android_notify_chager_info);
...
void msm_notify_chg_info_from_mhl_irregular(unsigned mA)
{
	...
	/* 0:disconnect other:connect */
	if (mA > 0) {
		shchg_api_notify_usb_charger_connected(SHCHG_DEVICE_IRREGULAR_CHARGER);
		shchg_api_notify_usb_charger_i_is_available(mA);
	}
	else {
		shchg_api_notify_usb_charger_disconnected();
...
static void msm_otg_start_host(struct usb_otg *otg, int on)
{
	...
	if (on) {
...
#ifdef CONFIG_USB_MSM_OTG_SH_CUST
		kobject_uevent_env(&motg->phy.dev->kobj, KOBJ_CHANGE, connected);
		pr_debug("sent host uevent %s \n", connected[0]);
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */

#ifdef CONFIG_USB_MSM_OTG_SH_CUST_EYEPATTERN
		msm_otg_set_override(ulpi_val_override_host);
		...
		kobject_uevent_env(&motg->phy.dev->kobj, KOBJ_CHANGE, disconnected);
		pr_debug("sent host uevent %s \n", disconnected[0]);
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
	}
}
...
#ifdef CONFIG_USB_MSM_OTG_SH_CUST
	if (atomic_read(&usb_host_en) == 0) {
		char *uevent_envp[]={"USBHOST=DISABLE",NULL};
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_PROPRIETARY_CHARGER:
					...
					break;
#if defined(CONFIG_USB_MSM_OTG_SH_CUST) && defined(CONFIG_USB_MSM_OTG_SH_CUST_CHARGE)
				case USB_PROPRIETARY_CHARGER:
					/* Enable VDP_SRC */
					ulpi_write(otg->phy, 0x2, 0x85);
					...
					pm_runtime_suspend(otg->phy->dev);
					break;
#endif /* defined(CONFIG_USB_MSM_OTG_SH_CUST) && defined(CONFIG_USB_MSM_OTG_SH_CUST_CHARGE) */
...
#if defined(CONFIG_USB_MSM_OTG_SH_CUST) && defined(CONFIG_SHSYS_CUST_PERFLOCK)
			if (!is_perf_lock_active(&usb_host_limit_lock)) {
				limit_lock(&usb_host_limit_lock);
				msleep(200);
			}
#endif /* defined(CONFIG_USB_MSM_OTG_SH_CUST) && defined(CONFIG_SHSYS_CUST_PERFLOCK) */
...
static bool msm_otg_id_check(void)
{
#ifndef CONFIG_USB_MSM_OTG_READ_USB_ID_FROM_SHBATT
	struct msm_otg *motg = the_msm_otg;
	return !irq_read_line(motg->pdata->pmic_id_irq);
#else /* CONFIG_USB_MSM_OTG_READ_USB_ID_FROM_SHBATT */
	shbatt_adc_t adc;
...
{
	size_t val;
	val = snprintf(buf, PAGE_SIZE, "%d\n", (int)shvbus_usb_force_disconnect);
	return val;
}
...
static ssize_t
usb_host_enable_store(struct device *_dev, struct device_attribute *attr,
			const char *buf, size_t size)
{
	int value;
	struct msm_otg *motg = the_msm_otg;
	...
	atomic_set(&usb_host_en, value);
	return size;
}

static ssize_t
usb_host_enable_show(struct device *sdev, struct device_attribute *attr, char *buf)
{
	size_t val;
	val = snprintf(buf, PAGE_SIZE, "%d\n", (int)atomic_read(&usb_host_en));
	return val;
}
...
int dwc3_otg_is_usb_host_running(bool isUnLock)
{
	...
	if (otg->phy->state >= OTG_STATE_A_IDLE && atomic_read(&usb_host_en) == 1) {
		dev_dbg(otg->phy->dev, "usb host running\n");
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
		...
		dev_dbg(otg->phy->dev, "usb host not running\n");
		return 0;
	}
}
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
...
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
static int msm_otg_notify_enumeration_started(struct usb_phy *otg)
{
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	atomic_set(&usb_host_en, 1);
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
	limit_lock_init(&usb_host_limit_lock, PERF_LOCK_998400KHz, DRIVER_NAME);
#endif /* CONFIG_SHSYS_CUST_PERFLOCK */
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
...
#ifdef CONFIG_USB_MSM_OTG_SH_CUST_CHARGE
	phy->otg->phy->notify_chg_info = msm_otg_notify_chg_info;
#endif /* CONFIG_USB_MSM_OTG_SH_CUST_CHARGE */
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
	phy->otg->phy->notify_enumeration_started = msm_otg_notify_enumeration_started;
#endif /* CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE */
	...
	ret = device_create_file(&pdev->dev, &dev_attr_usb_host_enable);
	if (ret < 0) {
		device_remove_file(&pdev->dev, &dev_attr_usb_force_disconnect);
		device_remove_file(&pdev->dev, &dev_attr_usb_cable_type);
		pr_err("%s: Failed to create the file entry(usb_host_enable)\n", __func__);
	}
	ret = device_create_file(&pdev->dev, &dev_attr_usb_host_running);
	if (ret < 0) {
		device_remove_file(&pdev->dev, &dev_attr_usb_force_disconnect);
		device_remove_file(&pdev->dev, &dev_attr_usb_cable_type);
		device_remove_file(&pdev->dev, &dev_attr_usb_host_enable);
		pr_err("%s: Failed to create the file entry(usb_host_running)\n", __func__);
	}
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
...
static int __devexit msm_otg_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_usb_force_disconnect);
	device_remove_file(&pdev->dev, &dev_attr_usb_host_enable);
	device_remove_file(&pdev->dev, &dev_attr_usb_host_running);
#endif /* CONFIG_USB_MSM_OTG_SH_CUST */
	cancel_delayed_work_sync(&motg->chg_work);
