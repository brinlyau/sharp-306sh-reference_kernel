...
extern void android_notify_chager_info(unsigned mA);
extern void prepare_to_check_selfpowered(struct usb_gadget *_gadget);
#endif
					...
					const char *buf, size_t size)
{
	...
	if (is_now > 0)
	{
		prepare_to_check_selfpowered(adev->cdev->gadget);
		is_selfpowered = usb_gadget_is_selfpowered(adev->cdev->gadget);
		if(is_selfpowered)
			power = 0;
		else
			power = IDEV_CHG_MIN;

		android_notify_chager_info(power);
	}
#endif
